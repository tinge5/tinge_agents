...
export class AuthService {
  ...
  async register(email: string, password: string, displayName: string) {
    ...
    return { user: { id: user.id, email: user.email, displayName: user.displayName }, accessToken: this.signAccessToken(user), refreshToken };
  }
  ...
  async refresh(refreshToken: string) {
    try {
      ...
      return { accessToken: this.jwt.sign({ sub: payload.sub }), refreshToken };
    } catch {
