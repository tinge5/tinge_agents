...
export class PlansService {
  ...
  async create(userId: string, dto: any) {
    ...
    return this.prisma.workoutPlan.create({
      data: { userId, name: dto.name, goals: dto.goals || [], progressiveOverloadEnabled: !!dto.progressiveOverloadEnabled, isActive: !!dto.isActive, status: dto.isActive ? 'active' : 'draft', currentWeekIndex: 0, days: { create: this.normalizeDays(dto.days) } },
  ...
  async activate(userId: string, planId: string) {
    await this.get(userId, planId);
    await this.prisma.$transaction([this.prisma.workoutPlan.updateMany({ where: { userId, isActive: true, id: { not: planId } }, data: { isActive: false, status: 'draft' } }), this.prisma.workoutPlan.update({ where: { id: planId }, data: { isActive: true, status: 'active' } })]);
