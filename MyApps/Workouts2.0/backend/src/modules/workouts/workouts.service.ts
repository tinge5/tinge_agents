...
import { applyProgression } from './exerciseProgression';
...
export class WorkoutsService {
  ...
  async today(userId: string) {
    ...
    const day = plan.days.filter((d) => Number(d.weekIndex ?? 0) === 0 || Number(d.weekIndex ?? 0) === weekIndex).find((d) => Number(d.dayOfWeek) === dow);
    ...
    return { status: 'scheduled', planId: plan.id, planDay: day, plan, weekIndex, title: day.title, day: `Day ${day.dayOfWeek + 1} (${DAY_LABELS[day.dayOfWeek] ?? 'Day'})`, exercises };
  }
