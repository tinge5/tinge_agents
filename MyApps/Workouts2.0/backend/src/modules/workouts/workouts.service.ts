...
function calculateWeekIndex(plan: { createdAt: Date; currentWeekIndex: number; startDate?: Date | null; durationWeeks?: number | null }, today = new Date()) {
...
function applyProgression(exercise: any, weekIndex: number, enabled: boolean) {
  ...
  const weightTarget = typeof exercise.weightTarget === 'number' ? Number((exercise.weightTarget * (1 + 0.025 * weekIndex)).toFixed(1)) : null;
  return {
    ...exercise,
    setsTarget: Math.max(1, Number(exercise.setsTarget ?? 0)),
    repsTarget: Math.max(1, Number(exercise.repsTarget ?? 0) + weekIndex),
    weightTarget,
  };
}
...
export class WorkoutsService {
  ...
  async today(userId: string) {
    ...
    const exercises = day.exercises.map((exercise) => applyProgression(exercise, weekIndex, plan.progressiveOverloadEnabled)).map((exercise) => ({
